...
function TaskRow({ taskItem, users, handleDeleteTask }: TaskRowProps) {
 ...
 const bgStatus = colorStatusTask(taskItem.status);
  ...
  return (
    <tr className={`bg-[#eceaee] hover:bg-[#d7d8d8] ${lexen.className}`}>
      <th className={`px-6 py-4 font-medium text-[#111111]`}>{taskItem.title}</th>
        ...
        <td className="px-6 py-4 flex items-center justify-center gap-2">
          {users && users.length > 0
            ? users.map((user) => (
                  ...
                  <div key={user.id} className="w-[40px] h-[40px] border-[#005C53] border-2 rounded-full flex justify-center items-center ">
                    <Image 
      ...
      <div className="flex gap-2">
          <Button
            ...
            className="text-black bg-[#9FC131] p-2 rounded-lg "
          >
            Update
          </Button>
          <Button
